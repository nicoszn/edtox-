...
import { mathSpanToHtml } from "@/lib/tools/inlineMath";

/*
 * Converts **bold**, *italic* / _italic_, `code`, [text](url), and $latex$
 * to inline HTML.
 *
 * Inline math is pulled out into placeholders BEFORE the other passes run,
 * for two reasons:
 *  1. The &/</> escaping pass would otherwise mangle the KaTeX-rendered
 *     HTML that mathSpanToHtml() inserts.
 *  2. The bold/italic regexes would otherwise misread LaTeX like `x_1*y_2`
 *     as markdown emphasis.
 * The rendered spans are swapped back in as the final step, after every
 * other regex has already run over the (now math-free) text.
 */
function inlineMdToHtml(text: string): string {
  ...
  const withPlaceholders = text.replace(
    ...
    (match, latex: string) => {
      ...
      mathTokens.push(mathSpanToHtml(clean));
  ...
  return formatted.replace(
    /\u0000MATH(\d+)\u0000/g,
    (_match, idx: string) => mathTokens[Number(idx)]
  );
}
